...
public class Word
{
  ...
  {
    int syllable = 0; //initializing a variable that holds vowel count
		int end = inputWord.length()-1; //last position in a word
		for ( int i = 0; i < end; i++)
		{
			//using isVowel to check to see how many indepent vowels there are
		 ...
		 // if the end is not an 'e' than we count that
		 if ( isVowel(inputWord.charAt(end)) )
		 {
		 	if ( inputWord.charAt(end) != 'e' && inputWord.charAt(end) != 'E')
				syllable++;
		 }
		 //if the word (according to flesch) has no vowels the whole thing
		 //counts as one
		 if ( syllable == 0)
		 	syllable = 1;
		 return syllable;
